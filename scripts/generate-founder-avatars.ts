#!/usr/bin/env ts-node
/**
 * Generates deterministic SVG avatar files for founders mirroring logic in Avatar.tsx
 */
import * as fs from 'fs';
import * as path from 'path';

interface FounderPreset { background: { from: string; to: string }; hair: { style: string } }
const founders: { name: string; preset: FounderPreset }[] = [
...
const HAIR_STYLES = ['buzz', 'short', 'wavy', 'curly', 'long', 'bun', 'bald'] as const;
const MOODS = ['neutral', 'smile', 'grin'] as const;
...
function derive(name: string): { skinTone: string; hairStyle: string; mood: Mood; hairColor: string } {
...
function hairPath(style: string, color: string): string {
  switch (style) {
    case 'bald':
      return '';
    case 'buzz':
...
for (const f of founders) {
  ...
  const first = f.name.split(' ')[0] || 'founder';
  const fileName = first.toLowerCase() + '.svg';
