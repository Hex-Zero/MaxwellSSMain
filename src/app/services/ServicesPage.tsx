	...
	return (
		<div className="container section" data-reveal>
			...
			<div className="mt-10 grid gap-6 md:grid-cols-3" data-reveal>
				<div className="flex flex-col gap-6">{services.slice(0,2).map(s => <ServiceCard key={s.key} s={s} />)}</div>
				<div className="flex flex-col gap-6">{services.slice(2,4).map(s => <ServiceCard key={s.key} s={s} />)}</div>
			...
			<div className="mt-20" data-reveal>
				...
				<p className="mt-3 text-sm max-w-2xl text-foreground/70">A lightweight, outcome-focused sequence that surfaces risk early and compounds engineering throughput.</p>
