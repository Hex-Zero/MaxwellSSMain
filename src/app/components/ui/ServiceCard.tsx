...
export function ServiceCard({ title, summary, icon, imageSrc, imageAlt, meta, tone = 'default' }: ServiceCardProps): ReactElement {
    ...
    >
      <div className="flex items-start gap-4">
        <IconChip
          icon={
            imageSrc ? (
              <Image src={imageSrc} alt={imageAlt || title} width={28} height={28} />
            ) : (
              icon || '🛠️'
            )
          }
        ...
        <div className="flex flex-col">
          ...
          {meta && (
            <p className="mt-3 text-[11px] uppercase tracking-wide text-[--muted]">{meta}</p>
          )}
        </div>
      </div>
    </article>
