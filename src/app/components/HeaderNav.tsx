...
export default function HeaderNav(): ReactElement {
  ...
  useEffect(() => {
    function onResize(): void { if (window.innerWidth >= 600) setOpen(false); }
  ...
  return (
    <nav className="container h-14 flex items-center justify-between relative" aria-label="Site header">
      ...
      <button type="button" className="menu-toggle" aria-label={open ? 'Close menu' : 'Open menu'} aria-expanded={open} aria-controls="mobile-nav-panel" onClick={toggle} data-test="menu-toggle">
